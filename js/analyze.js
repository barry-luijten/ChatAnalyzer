...
ca.re.sticker = new RegExp(/\u200esticker omitted/);
...
class ChatStatistics extends Statistics {
  ...
  calculateRanks() {
    this.ranks = {};
    var rankings = ['messageCount','wordCount','wordCountUnique','wordsPerMessage','emojiCount','links','images','documents','gifs','stickers','videos','audios','contacts','locations'];
...
function displayGroupStats(chatStats) {
  ...
  // personal STATS
    ...
    tableRows.innerHTML = //"<th scope='row'>"+"<h4 data-letters='" + content[i].name.match(/\b\w/g).join('') + "'></h4>"+"</th>" +
                          ...
                          "<td>"+stats.stickers + ca.getRankEmoji(chatStats.getUserRank(stats.name,'stickers')) + "</td>" +
